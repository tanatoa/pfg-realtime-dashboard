var taskrouterDashboard = new Vue({
  ...
  data: {
    ...
    loggedUser: "amcpfg_at_amc_dot_corp",
      ...
      return axios.get(this.tasks_get_url + '?userid=' + this.loggedUser)
        .then(function (response) {
          ...
          for (var i in tasks) {
            ...
            task['channel'] = tasks[i]['TaskChannelUniqueName'] ? tasks[i]['TaskChannelUniqueName'].charAt(0).toUpperCase() + tasks[i]['TaskChannelUniqueName'].slice(1) : 'Voice';
            ...
            //task['team'] =  tasks[i]['team'].charAt(0).toUpperCase() + tasks[i]['team'].slice(1);
            task['recordingUrl'] = tasks[i]['RecordingUrl'] ? tasks[i]['RecordingUrl'] : 'No Recording';
            taskSid = task['taskSid'];
            task['agentName'] = tasks[i]['WorkerName'] ? tasks[i]['WorkerName'] : 'Support';
            task['priority'] = tasks[i]['Priority'] ? tasks[i]['Priority'] : 'High';
